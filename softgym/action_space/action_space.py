...
class PickAndPlace(PickerQPG):
    ...
    def step(self, action):
        """ Action is in 4D: (u1,v1) the start of the pick in image coordinate; (u2, v2) the start of the pick in image coordinate"""
        u1, v1 = action[:2]
        ...
        st_high = np.array([x1, 0.2, z1, 0])
        st = np.array([x1, y1, z1, 0])
        en_high = np.array([x2, 0.2, z2, 1])
